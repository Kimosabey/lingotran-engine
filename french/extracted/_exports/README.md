...
| Pratique Conjugaison A1-A2 * | — | not yet processed | — | — | — |
| Pratique Revision 2 * | — | not yet processed | — | — | — |
...
- **Cosmopolite 1 - Methode de francais A1**: Question 201/5 (DELF A1 mock exam) is an IMAGE-CHOICE item: its three options are photographs, transcribed as descriptions, and the printed answer identifies one of them ("Une carte postale (photo a)"). It therefore can never match an option by text, and is listed in accepted_answer_flags so it stops blocking the gate while staying visible on every run. Not a defect and not fixable by re-extraction -- the options are pictures.
...
- **Tricolore 1 - 5e edition**: Two items (p046/1, p060/1) are typed multiple-choice but are listening tasks with neither options nor a printed answer; the item_type is likely wrong and is flagged for a repair pass, not silently corrected.
...
- **Pratique Conjugaison A1-A2**: ON THE LEGACY PIPELINE, NOT _engine/. This book predates the shared engine and uses the older per-book manifest system, so it produces none of the current deliverable artifacts and is absent from every French export. Registered here 2026-08-10 purely so reconcile.py can SEE it -- being invisible to the gates was the actual risk (gap P9), since an unregistered book reads as 'done' by omission.
- **Pratique Conjugaison A1-A2**: INCOMPLETE: 104 of 106 pages transcribed, and only 67 carry a QA verdict. It is neither finished nor formally parked. Decide before any delivery: finish it on the legacy pipeline, migrate it onto _engine/, or park it explicitly.
- **Pratique Revision 2**: ON THE LEGACY PIPELINE, NOT _engine/. Registered 2026-08-10 so the gates can see it (gap P9).
- **Pratique Revision 2**: NOT STARTED: 181 pages rasterized, 0 transcribed. No data of any kind exists for this book.
