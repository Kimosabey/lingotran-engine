...
(function () {
  ...
  /* ---- 6b. Shared topbar/mobile-nav render ------------------------------
     Every page keeps only <header id="topbar-root" data-active="slug"></header>
     — this builds the identical brand/nav/search/actions/mobile-nav markup
     from LT.sitePages so the 4 pages can't drift out of sync (a duplicated
     copy of this markup once shipped without a `defer` attribute on one
     page's script tag — see site/UX-AUDIT.md). */
  var SVG_SEARCH = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/></svg>';
  ...
  var SVG_SUN = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="4.5"/><path d="M12 2v2M12 20v2M2 12h2M20 12h2M5 5l1.5 1.5M17.5 17.5L19 19M19 5l-1.5 1.5M6.5 17.5L5 19"/></svg>';
  var SVG_MOON = '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20 14.5A8 8 0 019.5 4 8 8 0 1020 14.5z"/></svg>';
  var SVG_MENU = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M4 7h16M4 12h16M4 17h16"/></svg>';

  function renderTopbar() {
    var root = $("#topbar-root");
    if (!root) return;
    var active = root.getAttribute("data-active") || "home";
    var pages = LT.sitePages || [];
    var prefix = active === "home" ? "" : "../";
    function hrefFor(p) {
      if (p.slug === active) return "./";
      if (p.slug === "home") return "../";
      return prefix + p.path;
    }
    function linksHtml() {
      return pages.map(function (p) {
        return '<a href="' + hrefFor(p) + '"' + (p.slug === active ? ' class="active"' : "") + ">" + esc(p.label) + "</a>";
      }).join("");
    }

    root.innerHTML =
      '<a class="brand" href="' + hrefFor({ slug: "home" }) + '">' +
        '<img class="logo-light" src="' + prefix + 'assets/img/logo-color.png" alt="Lingotran">' +
        '<img class="logo-dark" src="' + prefix + 'assets/img/logo-white.png" alt="Lingotran">' +
        '<span class="kicker">Extraction Docs</span>' +
      "</a>" +
      '<nav class="appnav" aria-label="Primary">' + linksHtml() + "</nav>" +
      '<span class="spacer"></span>' +
      '<div class="search">' +
        '<span class="ico">' + SVG_SEARCH + "</span>" +
        '<input id="search-input" type="search" placeholder="Search sections…" aria-label="Search sections">' +
        "<kbd>/</kbd>" +
      "</div>" +
      '<div class="topbar-actions">' +
        '<a class="icon-btn" data-repo href="' + (LT.REPO_URL || "#") + '" target="_blank" rel="noopener" aria-label="GitHub repository">' + SVG_GITHUB + "</a>" +
        '<button class="icon-btn" id="theme-toggle" aria-label="Toggle light/dark theme">' +
          '<span class="sun">' + SVG_SUN + "</span>" +
          '<span class="moon">' + SVG_MOON + "</span>" +
        "</button>" +
        '<button class="icon-btn hamburger" id="mobile-nav-toggle" aria-label="Open navigation menu" aria-expanded="false" aria-controls="mobile-nav">' + SVG_MENU + "</button>" +
      "</div>";

    var scrim = document.createElement("div");
    scrim.className = "scrim"; scrim.id = "nav-scrim";
    var mobileNav = document.createElement("div");
    mobileNav.className = "mobile-nav"; mobileNav.id = "mobile-nav";
    mobileNav.innerHTML = linksHtml();
    // insertAdjacentElement("afterend") on the same anchor twice — the
    // second call lands directly after root, pushing the first one down —
    // so insert scrim first to get final order root, mobileNav, scrim.
    root.insertAdjacentElement("afterend", scrim);
    root.insertAdjacentElement("afterend", mobileNav);
  }
