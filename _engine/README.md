...
| `verify_answers.py` | Correct-answer alignment sweep, run after `merge_enrich.py` and before `build_exports.py`. Auto-fixes the one deterministic case (a multiple-choice `correct_answer` that's a bare letter gets rewritten to that option's full text); reports everything else that needs a human/agent read (an MC answer not found among its own options — often a 4th+ printed option the 3-slot schema couldn't hold — and inconsistent `(open-ended...)` formatting) rather than guessing. Also enforces the **inferred-level contract**: for a `level_mode: inferred` book, flags any question item missing a per-item `level` or carrying one outside `level_options` (aggregated, report-only — never guesses a level). |
