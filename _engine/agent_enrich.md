...
- `option_a` … `option_e`: MC/choice options verbatim, in printed order (else "").
  ...
  pointed at a column that did not exist (Cosmopolite p85 prints four; Tricolore 2 p53
  prints five). If a book ever prints more than five, capture the first five and say so
  in your report rather than dropping them silently.
