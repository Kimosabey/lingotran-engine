...
### Step 7b — The source of truth is the CHUNKS, always [read this]

Edit `pages/_questions/chunk-*.json`. Never `pages/_questions.json` — that file
is DERIVED, regenerated by `merge_enrich.py`, and a direct edit there is erased
with no error and no warning.

This used to differ per book, which was a live trap: French was uniformly
chunk-backed while German's five Goethe exam books had no chunk directory at
all, so their merged file genuinely WAS authoritative. Getting it backwards
destroyed work silently, and every agent had to be briefed per book. Closed on
2026-08-10 (gap P2) by giving those five books a chunk directory, so the rule
now has no exceptions to remember.
...
- After editing chunks you **must run `merge_enrich.py` before exporting**, or
  ...
  export at 8.7% rubric coverage that looked plausible enough to ship. Gate 4b
  (coverage) now catches exactly that.
...
| ~~P2~~ | ~~Source of truth is not uniform.~~ **CLOSED 2026-08-10.** The five Goethe exam books had no chunk directory, so their merged file was authoritative while every other book was chunk-authoritative -- a split that had to be briefed per book and destroyed work silently if reversed. All five were given a chunk directory, so the rule is now "always chunks", with no exceptions. | closed | closed |
| ~~P3~~ | ~~German's internal key is still `teil`.~~ **CLOSED 2026-08-10.** All 5,413 records renamed to `part` (0 conflicts), and the read-either compatibility shim removed from `questions.py`, so the key can no longer drift back. The VALUE stays verbatim German ("Teil 1") -- English column name, source-language content. | closed | closed |
| ~~P4~~ | ~~Questions carry only option_a-option_c.~~ **CLOSED 2026-08-10.** Schema runs `option_a`..`option_e` in both pipelines, the gate and the enrichment prompt; the two known truncated items were backfilled from their pages. `verify_answers` also now recognises a MULTI-SELECT answer (several options at once) instead of flagging it as unmatched. | closed | closed |
...
| ~~P6~~ | ~~No gender column.~~ **CLOSED 2026-08-10.** `gender` is DERIVED at export from the printed suffix (`automne (m)` -> `m`), so 403 French entries are now filterable while the headword stays byte-for-byte as the book prints it. Optional in the gate, since only some books mark gender. | closed | closed |
| ~~P7~~ | ~~Some columns are near-empty for some books.~~ **CLOSED 2026-08-10.** Each book's unified `.md` now states which vocabulary columns its source leaves empty ("`plural` is empty throughout -- this book prints none"), COMPUTED from the shipped rows so it cannot drift from the file. An SME can no longer mistake "the book prints none" for "the extraction missed it". | closed | closed |
| ~~P8~~ | ~~verify_answers is report-only.~~ **CLOSED 2026-08-10.** `--strict` makes it binding and CI uses it. Individually reviewed, legitimate flags are waived per book via `accepted_answer_flags` and still print every run -- the same discipline as `accepted_qa_gaps`. All three French books are CLEAN with strict on. | closed | closed |
| ~~P9~~ | ~~Two French books are invisible to every gate.~~ **CLOSED 2026-08-10.** Both registered with `pipeline: legacy-manifest`. `reconcile.py` reports them loudly with their real state but does not gate on them (they follow none of this engine's conventions), and `build_exports.py` excludes them from the deliverable. Visible-and-excluded is honest; invisible read as done-by-omission. | closed | closed |
...
| ~~A2~~ | ~~Agents have unrestricted write access.~~ **CLOSED 2026-08-10** as far as a prompt can. All three agent prompts carry a WRITE SCOPE block naming the exact paths that may be written and forbidding the derived files, other collections and the tooling directories. Still not *enforced* -- the prompt is the control, and it says so. | closed | closed |
| ~~A3~~ | ~~Disk-truth verification is a practice, not a tool.~~ **CLOSED 2026-08-10.** `_engine/verify_backfill.py` ships it: record counts unchanged, field present everywhere, and NOTHING ELSE CHANGED (parsed-JSON comparison, so a re-indent is not mistaken for drift). Handles both `items` and `entries`, and both chunk-backed and merged-file books. | closed | closed |
| ~~A4~~ | ~~Cross-agent convention drift.~~ **CLOSED 2026-08-10.** Every agent prompt now tells the agent to read an already-completed chunk of the same book against its pages first and match that convention, or to state the convention it is setting if it is first. Applied by hand during the backfills, it measurably made a book's chunks agree with each other. | closed | closed |
| ~~A5~~ | ~~No golden-page fidelity fixtures for transcription.~~ **CLOSED 2026-08-10.** `test_golden_transcription.py` freezes excerpts from real corpus pages -- a three-column bilingual glossary, a rubric-plus-items exercise page, and a German page with umlauts/ss -- and asserts the PROPERTIES a faithful transcription must hold (no lost table column, no lost accent, no lost item, no comment-as-title, bare level tag). Property-based rather than byte-equality, so legitimate layout differences do not cause false failures. | closed | closed |
